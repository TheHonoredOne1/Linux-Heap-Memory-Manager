...
void mm_instantiate_new_page_family(char *struct_name, __uint32_t struct_size)
{
    ...
    if (first_vm_page_for_families == NULL) // first family handling
    {
        ...
        strncpy(first_vm_page_for_families->page_family[0].struct_name, struct_name, MM_MAX_STRUCT_NAME);
        first_vm_page_for_families->page_family[0].struct_size = struct_size;
        return;
    }

    __uint32_t count = 0;
    ITERATE_PAGE_FAMILIES_BEGIN(first_vm_page_for_families, curr_family){
        
        if(strncmp(curr_family->struct_name, struct_name, MM_MAX_STRUCT_NAME) != 0)
        {
            count++;
            continue;
        }
    ...
    }ITERATE_PAGE_FAMILIES_END(first_vm_page_for_families, curr_family);
    ...
    if(count == MAX_FAMILIES_PER_VM_PAGE)
    {
        ...
        curr_family = &first_vm_page_for_families->page_family[0];
    }

    curr_family->struct_size = struct_size;
	strncpy(curr_family->struct_name,struct_name, MM_MAX_STRUCT_NAME);
    return;
}
